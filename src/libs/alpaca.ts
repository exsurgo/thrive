...
export class Alpaca {
  ...
  // Order methods
  createOrder(body) {
    return this.httpRequest("POST",this.argsFormatter("orders",undefined,undefined),JSON.stringify(body) as any).then((resp) => {
      return resp.json();
    }).then((resp) => {
      return resp;
    }).catch((err) => {
  ...
  // Bars methods
  getBars(path,query1,query2: any = undefined) {
