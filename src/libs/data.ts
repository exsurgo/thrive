...
import {ALPACA_SECRET, ALPACA_KEY, ALPHA_VANTAGE_KEY, STOCK_NEWS_KEY} from '../secure';
...
export async function getData(symbol: string, limit = 50): Promise<any> {
  if (TEST_DATA) return getTestData(symbol);
  const data = await alpaca.getBars('minute', symbol, {limit: limit})
  if (data && data[symbol]) {
    const prices = data[symbol].map((bar) => ({time: bar.t, value: bar.c}));
    return {
      prices,
      volumes: getRandomVolumeData(prices),
    } as IMarketData;
  }
}

function getRandomVolumeData(prices): IVolume[] {
  const volumes: any = [];
  for (const price of prices) {
    const volume = getRandomInt(1, 150);
    const color = volume < 75 ? 'red' : 'green';
    volumes.push({ time: price.time, value: volume, color: color });
  }
  return volumes;
}
...
export async function getCompanyNews(symbols: string[], limit: number): Promise<INews[]> {
  symbols.forEach(o => o.trim());
  const url = 'https://stocknewsapi.com/api/v1?' +
      `tickers=${symbols.join(',')}&items=${limit}&token=` + STOCK_NEWS_KEY;
...
export async function getCompany(symbol: string): Promise<ICompany> {
...
export async function getWatchList(): Promise<IStock[]> {
  return STOCKS;
}

export async function getStock(symbol: string): Promise<IStock> {
  return STOCKS.find(o => o.symbol === symbol)!;
}

/* Test Data */

const STOCKS: IStock[] = [
  {name: 'Apple', symbol: 'AAPL', price: getRandomInt(50, 150) },
  {name: 'Google', symbol: 'GOOG', price: getRandomInt(50, 150) },
  {name: 'Microsoft', symbol: 'MSFT', price: getRandomInt(50, 150) },
  {name: 'Facebook', symbol: 'FB', price: getRandomInt(50, 150), loss: true },
  {name: 'MGM Resorts', symbol: 'MGM', price: getRandomInt(50, 150) },
  {name: 'Amazon', symbol: 'AMZN', price: getRandomInt(50, 150) },
  {name: 'Netflix', symbol: 'NFLX', price: getRandomInt(50, 150), loss: true },
  {name: 'Adobe', symbol: 'ADBE', price: getRandomInt(50, 150) },
  {name: 'Zoom', symbol: 'ZM', price: getRandomInt(50, 150) },
  {name: 'Carnival', symbol: 'CCL', price: getRandomInt(50, 150), loss: true },
  {name: 'Southwest', symbol: 'LUV', price: getRandomInt(50, 150) },
  {name: 'Walmart', symbol: 'WMT', price: getRandomInt(50, 150), loss: true },
  {name: 'Clorox', symbol: 'CLX', price: getRandomInt(50, 150), loss: true },
  {name: 'Walt Disney', symbol: 'Disney', price: getRandomInt(50, 150) },
];

export async function getTestData(symbol): Promise<any> {
  return {
    prices: PRICE_DATA,
    volumes: VOLUME_DATA,
  }
...
/*
curl -X GET \
  -H "APCA-API-KEY-ID: PK4XRGWL4UAP09EOB768" \
  -H "APCA-API-SECRET-KEY: pFSPOefwPn1nYZYEAKP6qxGjSwiSGV93ehGm47pj"\
  https://paper-api.alpaca.markets/v2/account
*/

async function request<T>(path: string, params: any, isPost = false): Promise<T> {
  ...
  const resp = await fetch(`${HOST}/${path}${queryParams}`, {
